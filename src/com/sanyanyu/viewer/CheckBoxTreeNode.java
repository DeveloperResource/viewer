...
public class CheckBoxTreeNode extends DefaultMutableTreeNode {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8424889676783275026L;
		...
		if (_isSelected) {
			...
			if(parent instanceof CheckBoxTreeNode){
				...
				if (pNode != null) {
					...
					for (; index < pNode.children.size(); ++index) {
						if(pNode.children.get(index) instanceof CheckBoxTreeNode){
			...
			if (children != null) {
				...
				for (; index < children.size(); ++index) {
					if(children.get(index) instanceof CheckBoxTreeNode){
				...
				if (index == children.size()) {
					for (int i = 0; i < children.size(); ++i) {
						if(children.get(i) instanceof CheckBoxTreeNode){
